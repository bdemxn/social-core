...
function RegisterForm() {
	...
	const { register, handleSubmit, trigger, formState: { errors } } = useForm<User>({
					...
					<section className="flex flex-col gap-y-2">
						<div className="space-y-2">
							<div className="flex flex-col gap-y-1">
								...
								<Input id="fullname" type="text" placeholder="Kevin Bonilla" {...register("fullname")} />
								{errors.fullname && <span className="text-red-500 text-sm">{errors.fullname.message}</span>}
							</div>

							<div className="flex flex-col gap-y-1">
								<Label htmlFor="email">Email</Label>
								<Input id="email" type="email" placeholder="kevinb@social.core" {...register("email")} />
